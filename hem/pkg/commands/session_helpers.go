...
// addGadgetEnvironment supplies the relay identity through the agent process
// as a fallback for gadgets commands generated by an older Hem version.
func (e *Executor) addGadgetEnvironment(environment map[string]string) map[string]string {
	if e.MI6Control == "" || e.MI6ServerFingerprint == "" {
		return environment
	}
	...
	result["MI6_SERVER_FINGERPRINT"] = e.MI6ServerFingerprint
	return result
}

func (e *Executor) addGadgetEnvironmentValues(values *environmentValues) {
	if e.MI6Control != "" && e.MI6ServerFingerprint != "" {
		*values = append(*values, "MI6_SERVER_FINGERPRINT="+e.MI6ServerFingerprint)
	}
}
