...
func (c *client) getHistoryPaginated(sessionID string, count, from int) (*historyPage, error) {
	...
	return &historyPage{Conversation: result.Conversation, Total: result.Total, Revision: result.Revision, Generation: result.Generation}, nil
}

func (c *client) acknowledgeSession(sessionID string, revision, generation int64) error {
	resp, err := c.send("ack", "session", sessionID, "--revision", fmt.Sprintf("%d", revision), "--generation", fmt.Sprintf("%d", generation))
	if err != nil {
		return err
	}
	if resp.Status == protocol.StatusError {
...
func (c *client) listTraits() ([]traitInfo, error) {
	...
	if err := json.Unmarshal(resp.Data, &table); err != nil {
		return nil, fmt.Errorf("parsing traits: %w", err)
	}
...
func (c *client) showTrait(nameOrID string) (*traitDetail, error) {
	resp, err := c.send("show", "trait", nameOrID)
	if err != nil {
		return nil, err
	}
	if resp.Status == protocol.StatusError {
	...
	var detail traitDetail
	if err := json.Unmarshal(resp.Data, &detail); err != nil {
		return nil, fmt.Errorf("parsing trait: %w", err)
	}
	return &detail, nil
}
...
func (c *client) deleteTrait(nameOrID string) error {
	resp, err := c.send("delete", "trait", nameOrID)
	if err != nil {
		return err
	}
	if resp.Status == protocol.StatusError {
