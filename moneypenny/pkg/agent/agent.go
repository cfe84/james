...
func (r *Runner) runOpenCodeStreaming(cmd *exec.Cmd, buf *activityBuffer, sessionID string, stderrBuf *bytes.Buffer, persistTurns bool) (*Result, error) {
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &event); err != nil {
			r.vlog.Printf("opencode stream: unparseable line: %s", truncStr(line, 200))
			continue
		}
		...
		case "error":
			if errorData, ok := event["error"].(map[string]any); ok {
				if message, ok := errorData["message"].(string); ok {
					...
					streamErr = fmt.Errorf("opencode error: %s", message)
				}
			}
		}
	}
