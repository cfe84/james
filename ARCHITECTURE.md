...
The `gadgets` binary is a thin agent-facing HTTP client; Moneypenny, not the
client, owns its session identity, capability checks, and tool execution.
...
11. **Windows service task wrapper and watchdog**: Windows Task Scheduler limits an action command to 261 characters. Moneypenny service installation generates a `moneypenny-service.cmd` wrapper in the configured data directory containing the fully quoted Moneypenny command, a `moneypenny-service.vbs` launcher, and a UTF-16LE-with-BOM Task Scheduler XML definition. The native encoding avoids `schtasks` code-page switching failures on Windows installations that reject UTF-8 task files. User-level definitions use the current account SID obtained from `whoami /user`; using only `USERNAME` can cause Task Scheduler to reject an interactive-token import with access denied. The user logon trigger is scoped to that same SID and uses `StartWhenAvailable` so a missed login trigger is caught up. The wrapper records the command's exit status; after a nonzero exit it copies the now-closed daemon log to `crash-logs/moneypenny-crash-YYYYMMDD-HHMMSS-exit-N.log` and retains the ten most recent snapshots. It then returns the original exit status. The task invokes `wscript.exe //B` on that launcher: WScript starts `cmd.exe` with window style `0` (hidden), waits for it, and returns its exit code so Task Scheduler can distinguish an unexpected failure from a clean shutdown. Task Scheduler restarts a nonzero exit after one minute up to 999 times and disables its execution-time limit; zero exits are not restarted, so Moneypenny's auto-update helper can replace and launch the binary without a competing stale process. User services use a SID-scoped logon trigger; system services use a boot trigger and the LocalSystem account. This preserves long MI6 addresses, fingerprint pins, binary paths, and log paths without truncation.
...
### Gadgets platform and capability boundary (v1.78.0)

`gadgets/` is a standalone dependency-free Go module with
`cmd/gadgets`, built and installed alongside Moneypenny. It only parses commands,
reads stdin, posts JSON, and prints structured results; daemon code implements
all authorization, persistence, atomicity, and routing. See
[`gadgets/README.md`](gadgets/README.md) for the wire/CLI contract.

`handler/gadgets.go` binds `127.0.0.1:0` and serves non-browser POST requests at
`/gadgets`. `prepareGadgets` creates random session-bound bearer credentials,
keeps the credential maps in daemon memory, and injects `JAMES_GADGETS_URL` and
`JAMES_GADGETS_TOKEN` into the child environment. The daemon prepends its binary
directory to PATH so agents find the installed client. Credentials are neither
prompt content nor editable configuration; agent requests cannot specify source
identity. Closing the listener discards credentials, and a removed session is
rejected on the next capability lookup.

The endpoint accepts `{method,data}` and emits
`{success:true,data}` or `{success:false,error:{code,message}}`. It rejects
unknown request fields, trailing JSON, browser Origin headers, unauthenticated
requests, and unsupported methods; bodies and responses are bounded to 1 MiB
and 4 MiB respectively. The thin client's loopback-only HTTP transport rejects
proxies and redirects, pins `localhost` to `127.0.0.1`, has a 30-second request
deadline, and never retries writes. Client-side restrictions are defense in
...
guidance. Scheduling exposes only session-local list/create/delete; global
agent discovery/messaging and subagent operations route through Hem as described
below. Agents are instructed to use gadgets, never direct Hem, native memory
files, or direct database access. `agent/gadgets.go` grants non-yolo Claude only
`Bash(gadgets:*)`, and Copilot only `shell(gadgets:*)`, rather than the former
memory-directory/file-write grants. Native agent deny rules remain effective;
these grants do not promise every compound shell command or provider's permission
policy will allow execution. OpenCode retains its native permission behavior.

This is a **daemon API capability boundary, not an OS sandbox**. It does not
promise isolation from another process running as the same OS user or from a
yolo agent with broad native access. Operator Hem and the existing administrative
transports remain administrative; gadgets are not a retrofit of agent-scoped
authorization onto every Hem or Moneypenny command.

### Authoritative session memory (v1.78.0)
...
serializes operations per database without unbounded per-session bookkeeping,
avoiding SQLite journal/schema initialization races.
Gadget reads use Unicode-offset SQL slices of at most 64,000 characters, returning
the total size and optional next offset, so arbitrarily large imported notes
remain accessible despite the response cap. Runtime prompt inspection reads
only the bounded root slice and oversized-path/size metadata, not descendant bodies.

The empty path (`""`) is root; whole-tree listing returns root first, then
case-sensitive path order. `Children` selects immediate children. The logical
path normalizer rejects traversal segments, controls, invalid UTF-8, and
segments longer than 64 Unicode code points. Descriptions are retained when
present or derived from the first non-empty body heading/line. Search is
Unicode case-insensitive substring ranking over paths, body, and metadata,
not FTS or a vector index.

`Set` delegates to `SetBatch`. All incoming paths and bodies are validated
before a transaction: duplicate normalized paths, invalid UTF-8, or a body
over **4,000 Unicode characters** fail the entire batch. Replacements and
missing ancestor creation commit together. Each successful replacement
increments that node's **current** revision in SQL; caller-supplied revisions
are ignored. `memory.revisions` returns only `{path,revision}`. There is no
historical body store, historical revision replay, or optimistic
compare-and-swap API. Root deletion is refused; descendant checks and recursive
deletion happen in one transaction.

All existing Hem/TUI/Qew memory operations still use
`show/list/search/update/delete_memory`, whose handlers now delegate to these
same SQLite APIs. Thus operator and gadget edits share the 4,000-character
validation and transaction semantics, rather than maintaining competing stores.
The existing tree browser, single-note editor, search, and synthetic root row
are reused. The Markdown note field no longer implies direct README file access;
the TUI accepts the empty root path when saving an existing root note.
Same-host duplication calls `CopyTree` on the authoritative snapshot, never
...
`handler/memory_prompt.go` owns the shared contract for normal runs,
compaction, and distillation. `prepareRunInstructions` first prepares gadgets,
...
at **4,000 Unicode characters**. Oversized imported roots are clipped only in
the prompt, with a `gadgets memory get` pointer to their complete stored content.
The separate `oversizedMemoryWarning` scans node sizes and emits an at-most
**2,000-character warning** identifying individual over-limit nodes and their
counts, with bounded paths and an omitted-count/navigation footer. It is not an
aggregate-tree limit and does not warn merely because many small notes sum to a
large total. Neither injection nor warnings truncate persisted notes.

System rules tell agents to follow relevant branches, repair incomplete indexes
using immediate-child listing, split large notes into focused children, and
update parent/ancestor summaries atomically while retaining knowledge. The root
is a navigation snapshot, not a transcript or a promise that memory contains all
prior history. Instructions are not stored as memory content. Adapter delivery
remains Claude's system-prompt flag, Copilot's custom instructions file, and
OpenCode's task prefix. Notifications are independent of Memory; legacy
`schedule-system-prompt` is not an authorization control and the compatibility
schedule-tag parser remains, without advertising direct Hem commands.
...
Hem/Qew display Action needed before agent completion. It is a human-only
intervention path, never an agent-to-agent callback: prompt guidance directs
completion reports, review results, and blockers to the parent with
`gadgets subagents message`.
...
2. **Storage**: SQLite with WAL mode. The operational store owns session metadata/parameters/state/capabilities, ordered conversation history, queues, schedules, and other daemon records. Authoritative memory is separate: one `<sessionDir>/memory.db` per session, accessed through `pkg/memory`. Legacy operational memory rows are temporary import inputs, not a second live store. Operational connections use a five-second busy timeout, foreign keys on every connection, and immediate write transactions to avoid deferred read-to-write snapshot upgrades. The pool is bounded to four connections (one for in-memory stores).
...
- `create_directory` - Create a new directory (parents included) at `path`/`name`, returning its resolved absolute path
...
The source is bound by the authenticated daemon endpoint, not supplied by the
agent. A small wire client avoids a Moneypenny→Hem module dependency: local
requests use newline-delimited JSON over a Unix socket; remote requests use
`mi6-client --line-mode` with the daemon's `moneypenny_ecdsa` identity and the
persisted relay pin. Requests have a deadline, bounded responses and unique
correlation IDs; broadcasts are ignored and writes are not retried.

Hem strictly decodes method-specific data and consults its own current session
hierarchy. `subagents.list/message` include only direct children and the parent
(parent messages use the callback role); no sibling or descendant scope is
inferred. `agents.list/message` cover all tracked sessions but expose no
edit/delete operations. `subagents.create` fixes the parent/source server-side
and fetches the parent's current capabilities before using existing asynchronous
subsession creation. Agent-controlled prompt text is separated from operator
flags; callers cannot supply source identity, permission or route overrides.
...
23. **Scheduled continuation**: Moneypenny stores scheduled continuations in `schedules`. An asynchronous startup check and 30-second ticks process bounded batches (32 schedules, at most one per session). `Store.DispatchSchedule` uses a write-reserved transaction and a conditional snapshot claim to atomically advance the occurrence, claim an idle session or enqueue a delivery, and persist its conversation markers. Recurring schedules reuse the same row/ID with the next time computed after now; missed intervals are skipped. `prompt_queue.schedule_id` has a partial unique index for recurring deliveries, coalescing ticks while one delivery is queued. Cancellation removes that queued delivery; edits apply to future occurrences. Limits are 100 active schedules and 32 queued scheduled prompts per session; full queues defer rather than discard occurrences. Intervals below 30 seconds are rejected. Completed schedule history older than 30 days is pruned in small batches. `mark_ready` completion markers and channel routing are preserved. The existing TUI/Qew schedule components continue using the same IDs and protocol. Queue drains read at most 32 prompts/1 MiB per override/routing/Ready group (an oversized first prompt is delivered intact), with remaining work retained for later runs. An empty drain marks the session idle in that same transaction; the scheduler recovers idle queues using the same handler continuation path. Continuations launch in a new goroutine so prior run buffers are not retained on a recursive call stack. Operator and channel continuation paths use the same conditional idle claim. Legacy duplicate schedules and queues require explicit offline cleanup; no automatic data-loss migration is performed. See SPEC's backlog/WAL recovery procedure.
...
28. **Sub-agents**: Sub-sessions use the same Moneypenny session model, linked by `parent_session_id` in Hem's SQLite. Agents use `gadgets subagents create/list/message`; the authenticated daemon binds the source identity and routes through Hem, which enforces current direct-parent/child relationships and inherits capabilities at creation. Operator `hem create/list/watch subsession/session` workflows remain administrative. `watch session` uses an in-memory polling loop to queue completed results to the parent. Dashboard/session listings hide sub-sessions, parent deletion cascades, and TUI/Qew chat views show linked subagents. No direct-Hem command prefix, relay fingerprint, or key is injected into agent instructions.
...
35. **Agent traits**: Traits are reusable, hem-level system-prompt snippets (`id`, `name`, `prompt`) toggled on/off per session. They live entirely in hem (moneypenny is unaware), persisted in two SQLite tables: `traits` (definition) and `session_traits` (the `session_id`↔`trait_id` selection mapping, cleared on trait delete). Trait CRUD mirrors projects (`CreateTrait`, `GetTrait` by id-then-name, `ListTraits`, `UpdateTrait` with nil-pointer = skip, `DeleteTrait`, plus `SetSessionTraits`/`GetSessionTraits`).
    ...
    Trait composition is **compose-at-write**: selected trait prompts are rendered into the stored system prompt at create/update time. The effective order is **base → traits → gadgets → memory** (with nick first when present), but capability-filtered gadget instructions and current root memory are composed by Moneypenny at invocation time, not stored as fixed Hem command blocks. Arbitrary trait text is bounded by `<!--james:traits:begin-->`/`<!--james:traits:end-->` sentinels; `stripTraitsBlock`/`insertTraitsBlock` preserve surrounding markers. `resolveTraits` parses comma-separated IDs/names, de-duplicates in order, and rejects unknown traits.
...
35b. **Session nicknames**: An optional per-session **nickname** (hem-level alias) persisted in a new `sessions.nick` column (migrated via the idempotent `ALTER TABLE … ADD COLUMN` loop). Uniqueness is enforced case-insensitively in `SetSessionNick` (rejects a nick already held by another session; empty clears); `GetSessionByNick` does the reverse lookup. Nicks serve three roles:

    (1) **CLI selector.** A central resolver `resolveNickArg` runs in `Executor.Dispatch` (before the verb switch) for every command **except** the two assignment commands `create session` / `update session` (which read `--nick` as a value to write). It strips a `--nick VALUE`/`--nick=VALUE` token from the args, resolves it via `GetSessionByNick`, and rewrites it to `--session-id <id>` appended at the end — safe because `parseFlagsFromArgs` reorders flags before positionals, and essentially all session-targeting commands define `--session-id`. An unknown nick is a clear error; resolution is nick-only (no session-name fallback, per KISS).

    (2) **Display + filter.** The nick is appended as a trailing `Nick` column to the `list session` and `dashboard` `TableResult`s (trailing so existing positional clients don't break), read from the tracked session row (`sess.Nick`). The hem TUI parses the new column into `sessionInfo`/`dashboardEntry`/`sessionDetail` and renders it as a dim `nick · Name` prefix in the sessions list and dashboard; both `filteredSessions`/`filteredEntries` also match against the nick. Its chat view also prefers `sessionNick` over `sessionName` for assistant-message attribution. Qew mirrors this: `row[9]` → `e.nick`, a `.session-nick` span prefix, and the nick added to the dashboard fuzzy-match string; `renderChat` likewise prefers `currentSessionNick` for assistant-message labels.
...
35c. **Agent-to-agent message attribution**: The authenticated gadgets endpoint supplies the source James session ID server-side; identity is not trusted from an agent-provided `--from` flag or environment override. Hem resolves the source display label from nick/name/short ID and sends `source_session_id` and `source_name` on `ContinueSessionData` or `CreateSessionData`. These persist with conversation turns and queued prompts, preserving provenance for messages and initial subagent prompts. Parent replies use the `callback` role. Renderers prefer `source_name` over the human label; Qew retains its browser-local `qewUserLabel` preference. Legacy administrative Hem attribution support is not gadget authorization and is not advertised to agents.
...
54. **Numbered code-review marks (v1.33.0)**: Quick in-review navigation marks (1-9) in both the hem TUI git diff and the Qew git-diff review modal — in-memory only, never persisted. `Shift+N` sets mark N at the current position; `N` jumps back to it; the mark renders as a digit badge in the left gutter/margin. **Hem (`hem/pkg/ui/diff.go`):** `diffModel` gained `marks map[int]int` (mark→`scroll` vline offset) and `marksKey string` (the `selectedFile` the marks belong to, so switching between the whole-diff and a single-file view — which rebuilds the wrapped line set — discards stale marks automatically; marks also reset on `diffLoadedMsg`). In the diff-like key path, `shiftDigitMark` maps bubbletea's shifted-digit strings (`!@#$%^&*(`) to 1-9 for setting, and a plain `1`-`9` jumps (clamped). The gutter widened from 7 to 8 columns (5 line-number + 1 comment marker + **1 mark slot** + 1 space) so the mark digit (`markStyle`, blue bold) shows alongside the comment `*`; a reverse map (`vline index → mark number`) is built per render. **Qew (`qew/pkg/web/static/app.js` + `index.html`):** `diffReview.marks` maps mark→**global line seq** (so marks survive file-filter switches and are simply drawn only when their line is visible). `handleDiffModalKey` matches `e.code` `Digit1`-`Digit9` (layout-independent) — with Shift it calls `setMark` (records `visibleSeqs[cursor]`), without modifiers `jumpMark` (finds the seq in `visibleSeqs`, moves the cursor, scrolls into view). `applyMarkBadges()` clears and re-draws `.diff-mark` badges from the marks map after each render and after set/jump; `.diff-line` gained a `padding-left` gutter with the badge absolutely positioned in it. Both implementations deliberately differ only in mark-key scoping (hem resets on file-view change due to vline-index storage; Qew keeps them via seq keys) — behaviour is otherwise identical.

55. **Scoped (reviewed-files-only) commit & amend (v1.34.0)**: In both the hem TUI git-diff files tab and the Qew git-diff changed-files view, files can be toggled **reviewed** (ephemeral green `✔`). Previously Commit/Commit & Push/Amend always ran `git add -A` and committed the whole working tree. Now, when one or more files are marked reviewed, the commit is restricted to exactly those files. The `git_commit` protocol method (`moneypenny/pkg/envelope/data.go` `GitCommitData`) gained `Files []string`; the handler (`gitCommit`) stages with `git add -- <files>` (instead of `git add -A`) and appends a `-- <files>` pathspec to the commit/amend so it is a *partial commit* of just those paths (unrelated already-staged changes are excluded). The hem `commit session` command (`commands.go` `CommitSession`) gained a repeatable `--file` flag (reusing the existing `stringListFlag`), forwarded as `files` in the envelope. The hem TUI client `commitSession`/`amendSession` (`client.go`) take a `files []string` and emit `--file` per path; `doCommit` (`diff.go`) collects `m.reviewed ∩ m.fileList` (so stale marks for files no longer in the diff are dropped). Qew (`app.js`) adds a `reviewedFileArgs()` helper (intersecting `diffReview.reviewed` with `diffReview.fileList`) used by `submitCommit` and `gitAmend`; the Amend confirm dialog states whether it will stage "the N reviewed file(s)" or "all changes". With nothing marked, behaviour is unchanged (`git add -A`). Marks are in-memory only, so the scoping is a per-session, per-review-session convenience and is never persisted.

56. **Keyboard scroll/nav mode in conversation (v1.35.0)**: `j`/`k` (and `↓`/`↑`) now scroll the transcript while the message input is unfocused, in both surfaces. **Hem (`chat.go`):** the chat already has a `commandMode` (entered by the first `Esc`, which unfocuses the input) that previously only bound `enter`/`pgup`/`pgdown`/`ctrl+u`/`ctrl+d`; added `up`/`k` and `down`/`j` cases (3-line step vs the 10-line page step) routed through the same `m.scroll` offset (incl. older-history load when scrolling past the top). `j`/`k` reach this handler because ui.go's command-mode switch leaves them to its `default` (which falls through to `m.chat.Update`). The command-mode status hint now shows `j/k scroll`. **Qew (`app.js`):** the conversation's `Esc` opens a command-palette modal (the equivalent unfocused mode). Pressing `j`/`k`/arrows in the palette now calls `enterChatNavMode()` (sets `chatNavMode`, closes the palette via `closeWizard`, blurs `#chat-input`) and scrolls by `chatScrollLine` (~80px). A dedicated `chatNavMode` branch in the global keydown handler keeps `j`/`k`/arrows scrolling and exits on `Esc` (`exitChatNavMode(true)` refocuses the input); a `focus` listener on the input also clears `chatNavMode` so clicking back in is clean. The flag is reset on session open and whenever the palette (re)opens. A muted `j/k scroll transcript` hint was added to the palette footer. The mode is in-memory/UI-only; no protocol change.

57. **Scoped commit/amend includes the open single file (v1.35.1)**: the v1.34.0 scoped-commit feature only passed files explicitly marked **reviewed** (`Space`) as `--file` pathspecs. But the **Amend**/**Commit** buttons also appear in the *single-file diff view* (opened by `Enter` on a file, or auto-opened from the file list), where nothing is marked reviewed — so amending there fell back to `git add -A` and committed everything. Both surfaces now scope the commit to the **union** of the reviewed set and the currently-open single file: **Hem (`diff.go doCommit`)** adds `m.selectedFile` (deduped) to the reviewed paths; **Qew (`app.js`)** renamed `reviewedFileArgs()` → `commitFileArgs()`, which adds `diffReview.selectedFile` when in single-file (`mode === 'diff'`) view. The whole-tree-with-nothing-marked case still stages all changes. The Qew Amend confirm dialog now says "the N selected file(s)".

58. **Qew session title GUID fallback fix (v1.35.2)**: opening a chat in Qew without a name argument — via a deep-link/hash route (page reload, browser back/forward, or pasted `#/session/<id>` URL) or the parent-session stack (`openChat(parent, '', mp)`) — set the title to the truncated session GUID (`sessionId.substring(0, 12)`) because the name wasn't known client-side. Two fixes in `app.js`: (1) `openChat` now falls back to `sessionNameFromCache(sessionId)` (a lookup in the most recent dashboard payload `lastDashboardData.rows`, name = `row[1]`) before the GUID, so a cached name shows immediately with no flash; (2) `loadChat` backfills `currentSessionName` and the `chat-title` from `showResp.data.name` (the `get_session`/`ShowSession` `name` field) when the current title is still the GUID fallback, covering sessions not present in the dashboard cache. Genuinely-unnamed sessions keep the GUID. UI-only; no protocol change.

59. **Qew scheduled-tasks modal (v1.36.0)**: Qew gained a scheduled-tasks manager for the open conversation, mirroring the hem TUI's schedule picker (`chat.go`). Pressing `h` in the chat command palette (or **Actions → Scheduled Tasks**) opens `openSchedulesModal()` (`app.js`): it lists pending schedules (friendly local time via `formatScheduleTime`, a `↻ <cron>` badge for recurring ones, and the prompt), each with a **Delete** button (`cancel schedule <id> --session-id <sid>`), plus a **New task** form (a `datetime-local` seeded an hour ahead by `defaultScheduleLocal`, an optional cron field, and a prompt textarea) that calls `schedule session <sid> --at <ISO> [--cron <expr>] --prompt <text>` (the local time is converted to RFC3339 UTC via `toISOString`). Backed entirely by existing commands; the only backend change is that hem's `ListSchedules` now appends a **Cron** column to its table (and `ScheduleInfoResult` carries `cron_expr`) so recurring tasks are visible — safe for the TUI client, which only reads columns 0–3. Also fixed a latent Qew bug: the inline pending-schedule indicator mapped the `list schedule` table columns wrong (`r[1]` as session id, `r[4]` as status), so `status` was always `undefined` and the indicator never rendered; it now reads `[id, status, scheduledAt, prompt, cron]` correctly and formats the time. UI + one additive table column; no protocol change.

60. **"Add folder" in path browsers (v1.37.0)**: the create/duplicate wizard's path-selection step (hem TUI and Qew) gained the ability to create a new directory in place, plumbed down a new `create_directory` moneypenny method that parallels `list_directory`. **Moneypenny** (`handler.go createDirectory`, envelope `CreateDirectoryData{Path,Name}`/`CreateDirectoryResponse{Path}`): expands `~`/`~/`, joins `Name` onto `Path` (or treats `Path` as the target when `Name` is empty), rejects folder names containing `/`, `\`, `..`, or `.` to block traversal, `os.MkdirAll(target, 0o755)`, and returns the resolved absolute path. **Hem CLI** (`commands.go`): a hyphenated `create-directory` verb (routed in the early `if verb ==` block, like `list-directory`) → `CreateDirectory(noun,args)` with `-m`/`--path`/`--name`, returning `CreateDirectoryResult{Path}`. **Hem TUI** (`wizard.go`, `ui.go`, `client.go`): the path step renders a `+ Add folder` entry after the directories; selecting it enters an inline folder-name input mode (`creatingFolder`/`folderName`/`folderErr` state, rune/backspace/enter/esc handling that mirrors chat.go's schedule input — `updateWizard` routes Esc to the wizard model while creating instead of stepping back); on confirm `client.createDirectory` runs and the wizard navigates into the new folder and reloads. **Qew** (`app.js renderPathBrowser`): an `➕ Add folder` row at the bottom of `#wizard-dirs` `prompt()`s for a name, calls `create-directory`, then re-renders into the created path. Same dead-path/`~` semantics as the existing browser; additive protocol method, no breaking change.



61. **Copilot context-window tier (1M) + dead model-parser fix (v1.38.0)**: copilot exposes a context-window **tier** via the `--context <tier>` flag (`default` / `long_context`); `long_context` raises `max_context_window_tokens` to 1,000,000 for the *same* model id (the `-1m` model ids copilot lists are rejected as `--model` values, so this is not a model selection). The setting is plumbed end-to-end mirroring the existing `effort` override exactly (see decision #43). **Moneypenny**: `RunParams.ContextTier`; both copilot arg builders (`buildCopilotOneShotArgs`, `buildCopilotArgs`) append `--context <tier>` when non-empty (Claude builders untouched, ignore it). `sessions.context_tier` and `prompt_queue.context_tier` columns (schema + `ALTER TABLE` migrations); threaded through `Session`/`QueuedPrompt`, INSERT/SELECT/scan, `UpdateSessionFields`, `QueuePrompt`, both drain funcs, and the queue **group key** (`model, effort, context_tier`) so per-prompt overrides survive queueing. Envelope: `context_tier` on `CreateSessionData`/`ContinueSessionData`/`UpdateSessionData`(ptr)/`SessionDetail`. Per-prompt resolution: `effTier := sess.ContextTier; if data.ContextTier != "" { effTier = data.ContextTier }`. **Hem CLI**: `--context` on create/continue/copy/subsession, `--context none|default` clears on update, `SessionShowResult.ContextTier`, show-session table row. **Hem TUI**: create wizard (`wizard.go`) + edit form (`edit.go`) Context field (copilot-only options via `contextTierOptions()`, collapses to `[""]` for other agents via `applyContextTierOptions`), and a chat per-conversation override picker on command-mode `w` (`esc-w`) with a `🪟` header indicator (`chat.go`, `client.go`, `ui.go`). **Qew**: Context `<select>` in create/edit dialogs (copilot-only, repopulated on agent change in `syncWizardAgentDeps`), a header override dropdown + `esc-w` command-palette picker (`showOverridePicker('context')`), friendly labels (`default` / `1M (long context)`) with internal values (`long_context`), threaded into `continue`/`create`/`copy` API calls. **Parser fix**: `copilotModels()` was rewritten because copilot ≥1.0.69 no longer emits the `Listed models:` debug-log line (nor any structured catalog in the debug log). Stdout is now the robust primary source, validated by a new `copilotModelIDRe = ^[a-z][a-z0-9.]*-[a-z0-9.-]+$` (all real ids contain a hyphen, so prose/footer lines are dropped); the log parse is retained only as a best-effort source of display names. Additive schema/protocol; no breaking change.

62. **Local-time display (server sends raw UTC) (v1.38.1)**: timestamps in table results (dashboard, session list, subsessions) and chat turns were showing UTC on hosts whose server process runs in UTC. Root cause: hem's server pre-localized timestamps with `.Local()` (= the *server* process timezone) and stripped the zone to `"Jan 02 15:04"`, so browser/TUI clients could not re-localize. Fix moves localization to each **display** layer and sends **raw UTC RFC3339** (`...Z`) over the wire. **Hem server** (`commands.go`): the old `formatTimestamp` (which called `.Local()`) is replaced by `timestampForClient`, which normalizes any input to `2006-01-02T15:04:05Z`; used for the `Created`/`Last Active(ivity)` columns in the session-list and dashboard tables and the subsession rows (`sub.CreatedAt.UTC().Format(...)`). **Hem CLI** (`main.go`): the generic `TableResult` printer now runs `localizeTableTimestamps`, which localizes columns whose header is `Created`/`Last Active`/`Last Activity` back to `"Jan 02 15:04"` in the CLI host's timezone. **Hem TUI**: `relativeTime` (`dashboard.go`) already parses ISO correctly — `time.Since` is timezone-independent and the absolute fallback uses `.Local()` — so it now shows correct local relative times without change; the yearless `"Jan 02 15:04"` branch remains only as a legacy fallback. `chat.go`'s `localTime()` was already correct. **Qew** (`app.js`): `relativeTime` already localizes ISO input via `new Date(iso)`; a new `formatTurnTime(iso)` helper renders chat turn timestamps as local wall-clock time (`toLocaleTimeString`), replacing the previous raw-UTC display at the message role line. Schedule timestamps were already localized on both clients and are unchanged. No protocol/schema change — purely a display-layer fix.

63. **Copy-session summarizes with the _target_ agent (v1.45.0)**: `hem copy session` bootstraps the new session from a summary of the source's transcript, generated by a one-shot agent. Previously `CompactSession` (`moneypenny/pkg/handler/handler.go`) always ran that one-shot with the **source** session's agent (`sess.Agent`), so duplicating a Claude session to Copilot still invoked Claude first — and aborted the whole copy when the source agent was broken/unauthenticated (the failure surfaced as the opaque `agent oneshot failed: exit status 1 (stderr: )` because Claude prints its API error to stdout, which the success-path discards). Since the summary is pure text-processing of the stored transcript, it should use the agent the caller is copying *to*. **Moneypenny**: `envelope.SummarizeSessionData` gained optional overrides `agent`/`model`/`effort`/`context_tier`/`yolo` (`yolo` is a `*bool` to distinguish absent from explicit false). `CompactSession(ctx, sessionID, *summarizeOverrides)` now resolves effective agent params from the source session and then applies overrides — with the rule that a non-empty `Agent` override also forces `model`/`effort`/`context_tier` to come *only* from the overrides (not carried from the source, since they belong to a different model namespace); the copilot "prepend system prompt into the user prompt" branch keys off the *effective* agent. `summarizeSession` builds `*summarizeOverrides` from the envelope (nil when no fields set); the agent-side recovery path passes `nil`, preserving its use of the session's own agent. **Hem** (`commands.go CopySession`): sends the resolved target `agent`/`model`/`effort`/`context_tier`/`yolo` in the `summarize_session` payload. It also fixes the long-standing inheritance gotcha: `--model`/`--effort`/`--context` are no longer inherited from the source when `--agent` explicitly changes the agent (detected via a `flagSeen` helper), so a cross-agent copy doesn't carry an invalid source model into either the summary one-shot or the created session; the new agent picks its own defaults unless the flag is given explicitly. The one-shot still executes on the **source** moneypenny (which owns the transcript), so the target agent must be installed there — a cross-agent copy to an agent absent on the source MP errors clearly rather than silently degrading. Additive protocol (old moneypennies ignore the new fields and fall back to the source agent); minor version bump.

64. **Nickname in the chat/conversation title bar (v1.46.0)**: the session nickname (v1.44.0) already displayed in the dashboard, sessions list, and Qew dashboard; this extends it to the chat title bar in both clients. **Hem TUI** (`chat.go`): `chatModel` gained a `sessionNick` field and `historyLoadedMsg` a `nick` field; `loadHistory` reads `detail.Nick` from `showSession` (loaded lazily alongside status/context, so no threading through the 9 `newChatModel` call sites was needed) and the title render prefixes `nick · Name`, matching the dashboard/sessions convention. **Qew** (`app.js`): a `currentSessionNick` global plus a `sessionNickFromCache(id)` helper (dashboard `row[9]`) and a shared `setChatTitle()` that renders the nick with the existing `.session-nick` style (bold `#60a5fa`) before the name; wired into all three title-set sites (open, show-detail backfill, edit-save). Purely a display change — no protocol/schema change.

65. **Editable session path in Qew (v1.47.0)**: changing a session's working directory was already supported end-to-end — `hem update session --path`, the moneypenny `updateSession` handler (validates the new path exists via `os.Stat`, then persists through `UpdateSessionFields` — it only repoints, never moves/creates the folder), and the hem TUI edit form's **Path** field — but Qew's edit dialog rendered the path read-only (a `📂 {path}` badge). This adds an editable **Path** `<input id="es-path">` to the Qew edit dialog (`app.js`), submitted with `--path` only when changed (same changed-only pattern as the other fields), plus a hint that it merely repoints James to the folder (move it yourself first; the moneypenny rejects a non-existent path). No backend/CLI/TUI change. The intended use is recording a folder that was actually moved on the host — James never relocates files.

66. **Channel @mention defaults to the session nick (v1.48.0)**: when binding a channel to a nicknamed session, the @mention gate now pre-fills with the session's nick so the agent only replies when addressed by its nick — matching the identity block already injected into its system prompt. It's a **default, not a forced value**: clearing the field still forwards all messages. **Hem CLI** (`commands/channels.go CreateChannel`): a new package-level `flagSeenIn(args, names...)` helper (extracted from the closure previously inline in `CopySession`) detects whether `--mention` was supplied; when absent, the gate defaults to `store.GetSession(sessionID).Nick`; an explicit `--mention ""` is respected as forward-all. **Hem TUI** (`ui/channels.go`): `channelsModel` gained a `sessionNick` field (passed from `ui.go` via `m.chat.sessionNick`); the two create-flow entry points (search-pick and by-id) seed `mentionInput` with the nick instead of empty (the edit flow keeps the channel's stored mention). **Qew** (`app.js`): `renderGatingStep` pre-fills the mention input with `currentSessionNick`; `createChannel` now **always** sends `--mention` (even empty) so a cleared prefill overrides the CLI's nick-default rather than silently re-applying it. Edit/`set channel` is unchanged and never re-applies the nick.

67. **Cron parser supports lists, ranges, and steps (v1.48.1)**: the recurring-schedule cron evaluator (`moneypenny/pkg/handler/handler.go nextCronTime`) previously parsed each field with `fmt.Sscanf(s, "%d", &val)`, which **silently truncated** any list/range/step term — `9,13,17` became `9` and `1-5` became `1`. Because `fmt.Sscanf` returns no error when it stops at a `,` or `-`, both creation-time validation and recurrence computation accepted the malformed expression, so a cron like `0 9,13,17 * * 1-5` (weekdays at 09:00/13:00/17:00) silently degraded to `0 9 * * 1` (Mondays 09:00 only). The first occurrence is unaffected because it uses the user-supplied `--at` time; the bug only surfaced on the *next* recurrence computed by `scheduleNextCron`. The `parseField` closure was replaced with a package-level `parseCronField(s, min, max)` that implements standard cron term syntax: `*`, `*/n`, `a`, `a-b`, `a-b/n`, `a/n`, and comma-separated lists of those, returning a sorted, de-duplicated set (sorting matters because the next-occurrence loop returns the first candidate `After(after)`). Day-of-week now accepts `0-7` with `7` normalized to `0` (Sunday). Invalid terms (empty, non-numeric, zero/negative step, reversed range, out-of-range) now return errors instead of being silently accepted. Covered by `pkg/handler/cron_test.go`.

68. **Edit schedule in place (v1.49.0)**: pending schedules can now be edited without losing their ID (previously the only options were cancel + recreate). **Store** (`moneypenny/pkg/store/store.go`): `UpdateSchedule(id, prompt, scheduledAt, cronExpr, replyChannelID)` runs `UPDATE schedules SET … WHERE id=? AND status='pending'`, so it preserves the row/ID/created_at and refuses non-pending rows (0 rows affected → error); covered by `pkg/store/schedule_test.go`. **Protocol/handler**: `envelope.UpdateScheduleData`/`UpdateScheduleResponse` and the `update_schedule` method (`handler.go updateSchedule`) mirror the `schedule` handler — it validates the schedule exists and is pending, parses `scheduled_at` (RFC3339) and any `cron_expr` (via `nextCronTime`), then calls `store.UpdateSchedule` and emits a `chat_schedule` notification with `action:"updated"`. **Exact list data**: to prefill an edit form accurately, `list_schedules` already returned full `ScheduleInfo` (untruncated prompt, cron, channel), but hem's `ListSchedules` only surfaced a truncated display **table**. It now returns `ScheduleTableResult{headers, rows, schedules}` — the CLI still renders the (57-char-truncated) table because it keys off the `headers` field and ignores `schedules`, while UIs read the exact `schedules` array. `ScheduleInfoResult` gained `reply_channel_id`. **Hem CLI** `edit schedule SCHEDULE_ID --session-id ID [--at] [--prompt] [--cron] [--channel]` (`commands.go EditSchedule`): fetches the current schedule via `list_schedules` and merges — any flag not present in `args` (detected by scanning for `--flag`/`--flag=`) retains the current value, so a single field can change; `--cron ""`/`--channel 0` clear. **Hem TUI** (`chat.go`): the schedule picker gained an `e` binding that reuses the existing two-step scheduling input (`scheduling`), prefilled with the current time then prompt, dispatching `editSchedule` (client `edit schedule`) instead of `createSchedule` when `editingScheduleID != 0`; cron/channel are retained because they aren't passed. The TUI `scheduleInfo` client parse now prefers the structured `schedules` array (full prompt) over the truncated table rows. **Qew** (`app.js`): each pending row gained an **Edit** button that reopens the shared task form (`renderForm(editSched)`) prefilled from the exact schedule (time via new `isoToScheduleLocal`, cron, channel select, full prompt); **Save** calls `edit schedule … --at --cron --channel --prompt` (all fields, so cron/channel can be changed or cleared), while **New task** still calls `schedule session`. Additive protocol — old moneypennies simply lack `update_schedule`; the extra response field is ignored by old clients.

69. **Manual "mark as ready" / unread (v1.50.0)**: the dashboard's READY attention group already means *idle + `reviewed=false`* (a fresh agent response the user hasn't seen); this adds a way to put a session back into that group on demand — an "mark as unread" for conversations you want to revisit. It is a **pure hem-store flag flip** with no moneypenny/protocol involvement (`store.SetSessionReviewed` already existed; it drives the READY/IDLE split in `ListSessions`/dashboard). **Hem CLI**: `mark session SESSION_ID [--read]` (`commands.go MarkSession`, routed at the `case "mark session"`) clears `reviewed` (→ ready) or sets it with `--read`. **Hem TUI** (`ui.go`): a `u` binding on the dashboard, project-detail, and sessions-list selected row, and in chat command mode, calls a `markReady` client cmd (`dashboard.go`/`sessions.go`) that sends `mark session`; the dashboard/sessions variants reuse the existing `sessionCompletedMsg`/`sessionStoppedMsg` reload messages, while the chat variant uses a new `chatMarkedReadyMsg` that pops back to the previous list view (so the session is visible in the READY group) and refreshes. Status-bar hints gained `u ready` in all four contexts. **Qew** (`app.js`): a `markReady(sessionId)` mirror of `completeSession`; a **Mark as ready** command-palette item (`u`), the palette key-map entry `u:'markready'`, the `runCmd` case, plus dashboard row support via `dashAction('markready')` and the dashboard key handler `case 'u'`. Opening a chat still auto-marks it reviewed (read), so marking ready is the deliberate inverse. No schema/protocol change; old stacks simply lack the `mark session` verb until upgraded.

70. **Chat turn timestamps show a date when not today (v1.50.1)**: per-message timestamps in the chat transcript previously rendered wall-clock only (`15:04:05`), which is ambiguous once a conversation spans multiple days. Both clients now prefix a friendly date for non-today turns while keeping same-day turns time-only. **Hem TUI** (`chat.go localTime`): after localizing, it compares the local `Date()` against today and yesterday — same day → `15:04:05`, yesterday → `Yesterday - 15:04:05`, otherwise `Jan 02 - 15:04:05` (with `Jan 02 2006 - …` when the year differs). **Qew** (`app.js formatTurnTime`): the same logic via a `sameDay` helper and `toLocaleDateString` (`Yesterday - …`, `Aug 21 - …`, or `Jan 02, 2025 - …` across years). Purely a display-layer change — timestamps are still stored/sent as raw UTC RFC3339 (see #62); no protocol/schema change.

71. **Numbered subagent navigation in Qew (v1.51.0)**: the hem TUI lets you jump to the Nth subagent by pressing `1`-`9` in chat command mode; Qew now has parity. Qew's equivalent of TUI command mode is the **command palette** (opened with `Escape` in a conversation). When the open session has subagents, the palette renders a **Subagents** section listing them with `1`-`9` `<kbd>` shortcuts (built from the already-loaded `lastSubagents`, capped at 9), and the palette keydown handler opens `lastSubagents[n-1]` via the existing `window._openSubagent` (which pushes the parent onto `parentSessionStack` and opens the sub) when a digit `1`-`9` is pressed; clicking a numbered entry does the same through a `data-sub-idx` click branch. No new data is fetched — subagents were already displayed inline in the transcript (`subagent-indicator` rows) and cached in `lastSubagents`; this only adds the keyboard/click affordance in the palette. No protocol/store/CLI change.
...
73. **Subagent callbacks as distinct turns**: A subagent replies using `gadgets subagents message PARENT_ID`; the daemon supplies its source identity and Hem checks the current hierarchy. This parent-reply path is unconditional and remains injected even when the revocable Subagents capability is disabled. When that capability is enabled, Moneypenny's injected prompt defines James subagents as real Hem/Moneypenny child sessions, requires `gadgets subagents create` rather than generic task tools or runtime/background delegation, requires post-create verification through `gadgets subagents list`, and directs child/parent communication through `gadgets subagents message`. An independent `agents.message` invocation creates a durable, one-time reverse callback lease in Hem for that invoked session and invoker; the invoked session may use the same subagents message command to return one result, then Hem consumes the lease after successful delivery. This avoids a broad peer-to-peer grant while allowing invoked agents to report back. `messageGadgetAgent` attaches `source:"callback"` for parent and invocation replies, preserves source ID/name, marks the destination unreviewed/reactivates completed sessions, and tries `continue_session`, falling back to `queue_prompt` only when busy. Moneypenny stores the callback role both for direct delivery and when draining the queue. Operator `hem callback session ... --from ...` and `watch session` remain supported administrative flows, but their commands/identity flags are not agent instructions. TUI and Qew render callbacks compactly with a light-gray ↩️ marker and the originating subagent name; they always show them regardless of the train-of-thought toggle. The role uses existing `prompt_queue.source` and conversation-turn `role` fields.

74. **Completed subagents are hidden from live chat (`v1.54.0`)**: a parent conversation's inline subagent status rows and numbered navigation are now scoped to non-completed children, preventing finished tasks from crowding active work. The dashboard applies the same completed-only exclusion: every active child, including a reviewed idle child, is shown directly after its parent; only completed children require `--show-subs`. The full child list remains available deliberately: **Hem TUI** keeps the existing `Esc` → `a` subagent picker as an explicitly labelled **All subagents** screen (including completed entries), while **Qew** retains both `lastSubagents` (active-only, for inline rows and `1`–`9`) and `allSubagents` (unfiltered). Qew's command palette adds `l` → **All subagents**, a modal list with each child's name/status that opens any entry, including completed work. Opening the modal starts a deduplicated authoritative `list subsession` request rather than relying on the 750ms optional request in a chat refresh; it initially displays loading and then replaces its rows only if its session and chat generation remain current. Errors are shown in the modal with an explicit retry action. `allSubagentsCursor` drives its clamped `j`/`k` and arrow-key selection, `Enter` reuses `openAllSubagent()` to open it, and generic modal Escape invokes the Close button. The backend `list subsession` response is unchanged; filtering happens only at the respective live-chat presentation boundary.
...
76. **Windows release Authenticode signing (`v1.55.0`)**: GitHub Actions cross-compiles all platform artifacts as before, then a dedicated `sign-windows` job on `windows-2022` downloads the unsigned Windows archive, extracts it, and authenticates to Azure Artifact Signing via GitHub OIDC (`azure/login@v3` with repository variables). The job targets the stable `artifact-signing` GitHub environment because release workflows run from variable tag refs; that gives Entra one durable federated subject rather than a credential per version tag. It signs every `.exe` using the `github-signing` public-trust certificate profile. The signing action is restricted to the Azure CLI credential established by that OIDC login; no client secret or private signing key is stored in GitHub. Each signature is verified with `Get-AuthenticodeSignature` before the signed directory is recompressed. The release job depends on signing and uploads Linux/macOS build artifacts plus **only** `james-windows-amd64-signed.zip`, never the unsigned Windows archive. The workflow needs `id-token: write` only in the signing job; Azure RBAC grants the federated service principal `Artifact Signing Certificate Profile Signer` on the signing account/profile.
...
79a. **Protocol log metadata only (`v1.83.3`)**: Moneypenny logs request and response metadata (request ID, method, status, and byte count), never serialized protocol bodies. This prevents customer content and attachments from entering daemon logs and avoids recursive amplification when a `get_logs` response contains the daemon's own protocol output. Conversation pagination also has a composite `(session_id, created_at, id)` index so recent-history queries avoid sorting the entire session history.
79b. **Bounded agent diagnostics (`v1.83.4`)**: Agent subprocess stderr is captured in a fixed-size tail buffer, and activity summaries are truncated before entering the per-session ring buffer. This preserves actionable failure context and recent UI state without allowing noisy subprocesses or oversized streamed events to retain unbounded memory in Moneypenny.
...
86. **Qew local diff-redundancy hints (`v1.60.1`)**: Qew derives a lightweight potential-duplication signal entirely in the browser from working-tree diff additions. It normalizes whitespace, omits short, punctuation-only, and single-word additions from line-level matching, then detects repeated line patterns and exact repeated sequences of three meaningful added lines. The Files view reports aggregate counts and per-file `↻ NL MB` badges. This is deliberately an exact-match review hint, not semantic clone detection: it has no backend call, persists no source text, and cannot claim that repeated code is necessarily incorrect.
...
92. **Gadget session editing permissions**: `edit-own-session`, `edit-sessions`, and `edit-own-subagents` are independent opt-in capabilities. Hem TUI and Qew use the same explicit capability fields for create/copy/edit forms. Moneypenny checks the live capability set before routing. Hem authorizes the authenticated session, any tracked session, or a direct child respectively, decodes only editable session fields, and explicitly rejects gadget-capability and trusted-route changes. `edit-own-subagents` also permits direct-child lifecycle operations (complete, stop, delete). Replying to the parent is unconditional; child management is not.

93. **Qew transient connection recovery**: `app.js` centralizes connectivity in `setConnectionState`, which updates the header status and disables `#chat-send` whenever the API is unavailable or a send is in flight. `loadChat` treats history errors (including HTTP 502 during deployment) as transient: it leaves the current `#chat-messages` DOM and conversation state untouched, marks Qew disconnected, and relies on the existing 3-second `startChatPoll` loop to retry. A successful refresh marks the connection restored and re-enables Send. Dashboard request failures use the same status indicator, avoiding replacement of an active conversation with an error screen.

94. **Explicit Qew development mode (unreleased)**: Unauthenticated Qew operation is an explicit local-development exception. The `--development` flag is accepted only with a loopback listen address (`127.0.0.1`, `::1`, or `localhost`), preventing an accidental network-exposed no-password server. The Docker entrypoint requires `QEW_PASSWORD` and never adds `--development` implicitly; unauthenticated development runs must be started directly with an explicitly loopback-bound address.
...
building the agent subprocess environment. Updating replaces the complete set,
which makes secret rotation deterministic. Values are excluded from session
lists and runner logs, but returned to authenticated session-edit clients so
they can be reviewed and changed; they are never propagated to the Moneypenny
...
The Qew conversation header owns schedule visibility rather than the message
stream. Its schedule toggle opens the existing schedule modal, and a DOM-local
red dot reflects the latest pending schedule count from the normal chat poll
or a modal edit. This removes schedule metadata from conversation rows while
leaving scheduled invocation turns and the existing command-palette/action-menu
entry points intact.
...
- **Env vars**: `HEM_MI6_URL` (MI6 address for Hem control channel), `MI6_SERVER_FINGERPRINT` (required SHA-256 relay-server key pin when `HEM_MI6_URL` is set), `QEW_PASSWORD` (required for the container's authenticated Qew web UI), `LISTEN` (default `:8077`).
...
the configured system instructions. Memory access and injection are controlled
