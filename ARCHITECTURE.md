...
92. **Gadget session editing permissions**: `edit-own-session`, `edit-sessions`, and `edit-own-subagents` are independent opt-in capabilities. Moneypenny checks the live capability set before routing. Hem authorizes the authenticated session, any tracked session, or a direct child respectively, decodes only editable session fields, and explicitly rejects gadget-capability and trusted-route changes. `edit-own-subagents` also permits direct-child lifecycle operations (complete, stop, delete). Replying to the parent is unconditional; child management is not.
