...
92. **Gadget session editing permissions**: `edit-own-session` and `edit-sessions` are independent opt-in capabilities. Moneypenny checks the live capability set before routing `sessions.edit`; Hem then authorizes the target session, decodes only editable session fields, and explicitly rejects gadget-capability and trusted-route changes. The broader capability includes the authenticated session; the narrower capability cannot edit another session.
