...
func TestParseCommands(t *testing.T) {
	tests := []struct {
		...
		{"subagents edit", []string{"subagents", "edit", "id", "--model", "m"}, "", "subagents.edit", `{"session_id":"id","model":"m"}`},
		{"subagents complete", []string{"subagents", "complete", "id"}, "", "subagents.complete", `{"session_id":"id"}`},
		{"subagents stop", []string{"subagents", "stop", "id"}, "", "subagents.stop", `{"session_id":"id"}`},
		{"subagents delete", []string{"subagents", "delete", "id"}, "", "subagents.delete", `{"session_id":"id"}`},
...
func TestParseErrors(t *testing.T) {
	for _, args := range [][]string{
		...
		{"agents", "create"},
		{"moneypenny", "logs", "extra"}, {"moneypenny", "logs", "--lines=0"},
		{"moneypenny", "logs", "--lines=-1"}, {"moneypenny", "logs", "--lines=abc"},
		{"moneypenny", "logs", "--path=secret"}, {"moneypenny", "logs", "--session-id=other"},
		{"moneypenny", "logs", "--name"}, {"moneypenny", "logs", "--address=relay/session"},
