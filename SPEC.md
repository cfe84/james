...
- **Memory (Qew)**: The chat Actions menu, the command-palette action `m` (i.e. `esc-m`), open an editable **Memory** modal (a 95% large modal whose textarea fills the height; Save / Cancel) backed by `show memory` / `update memory`, mirroring the hem TUI memory view. Saving persists the note (injected into the agent's context on subsequent turns) and refocuses the input; an empty memory shows as blank.
