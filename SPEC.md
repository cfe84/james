...
Method: **git_commit**: stages all changes and commits in a session's working directory. Data: `{ "session_id": "id", "message": "commit message", "amend": false, "no_edit": false }`. Runs `git add -A` then `git commit -m` (or `git commit --amend -m` when `amend`, or `git commit --amend --no-edit` when `amend` and `no_edit` — which reuses the previous commit message, so no message is required). Returns `{ "output": "..." }`.
